...
f.move(folder=f.get_env().WindowsApps)"
...
f = f.joinpath(f'bat-{f[-1]}-x86_64-pc-windows-msvc.zip').download().unzip(inplace=True).joinpath(f'bat-{f[-1]}-x86_64-pc-windows-msvc/bat.exe').move(folder=f.get_env().WindowsApps)
"
