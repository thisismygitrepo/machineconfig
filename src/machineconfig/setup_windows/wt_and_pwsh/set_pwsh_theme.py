...
def install_nerd_fonts():
    ...
    if not isinstance(folder, P): raise ValueError(f"Failed to get latest release. Expected a Path object, got {folder}")
...
def change_shell_profile():
    ...
    shell = "pwsh"
    profile_path = Terminal().run("$profile", shell=shell).op2path()
    assert isinstance(profile_path, P), f"Could not find profile path for {shell}."
    local_app_data = os.getenv("LOCALAPPDATA")
    if not isinstance(local_app_data, str):
        raise ValueError("Could not find LOCALAPPDATA environment variable.")
    theme_path = P(local_app_data).joinpath(r"Programs\oh-my-posh\themes").collapseuser().as_posix().replace("~", "$env:USERPROFILE")  # organization machine with homeshare confuse H: with ~.
    txt = f"""
