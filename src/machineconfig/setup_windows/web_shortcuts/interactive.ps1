...
function devops   { & "$HOME\.local\bin\uvx.exe" --python 3.14 --from "machineconfig>=8.80" devops $args }
...
function terminal { & "$HOME\.local\bin\uvx.exe" --python 3.14 --from "machineconfig>=8.80" terminal $args }
function msearch  { & "$HOME\.local\bin\uvx.exe" --python 3.14 --from "machineconfig>=8.80" msearch $args }
