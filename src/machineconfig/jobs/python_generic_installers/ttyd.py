...
def main(version: Optional[str] = None) -> None:
    if platform.system() == "Windows":
        release = get_latest_release(repo_url=url, exe_name="ttyd", version=version, download_n_extract=False, linux=False)
        ...
        release = get_latest_release(repo_url=url, exe_name="ttyd", version=version, download_n_extract=False, linux=True)
        if not isinstance(release, P):
            print(f"Could not find browsh release for version {version}")
            return None
