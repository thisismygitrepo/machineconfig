...
def mail_public_from_parser(method: Literal["symlink", "copy"] = typer.Option(..., help="Method to use for setting up the config file."),
    ...
    items_objections: dict[str, list[SymlinkMapper]] = {item: mapper_full[item] for item in items_chosen if item in mapper_full}

    return items_objections


def mail_private_from_parser(method: Literal["symlink", "copy"] = typer.Option(..., help="Method to use for linking files"),
                             on_conflict: Literal["throwError", "overwriteSelfManaged", "backupSelfManaged", "overwriteDefaultPath", "backupDefaultPath"] = typer.Option("throwError", help="Action to take on conflict"), which: Optional[str] = typer.Option(None, help="Specific items to process"),
                             interactive: bool = typer.Option(False, help="Run in interactive mode")):
