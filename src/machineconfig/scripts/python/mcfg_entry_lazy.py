...
def _devops_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real devops app."""
    from machineconfig.scripts.python.devops import get_app
    real_app = get_app()
    real_app(args if args else ctx.args, standalone_mode=False)


def _cloud_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real cloud app."""
    from machineconfig.scripts.python.cloud import get_app
    real_app = get_app()
    real_app(args if args else ctx.args, standalone_mode=False)


def _sessions_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real sessions app."""
    from machineconfig.scripts.python.sessions import get_app
    real_app = get_app()
    real_app(args if args else ctx.args, standalone_mode=False)


def _agents_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real agents app."""
    from machineconfig.scripts.python.agents import get_app
    real_app = get_app()
    real_app(args if args else ctx.args, standalone_mode=False)


def _utils_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real utils app."""
    from machineconfig.scripts.python.utils import get_app
    real_app = get_app()
    real_app(args if args else ctx.args, standalone_mode=False)


def _terminal_proxy(ctx: typer.Context, args: list[str] | None = None) -> None:
    """Lazy proxy that loads and invokes the real terminal app."""
