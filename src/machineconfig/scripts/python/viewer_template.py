...
with col_info1:
    st.info(f"Dataset: {data_path.split('/')[-1]}")
with col_info2:
    st.info(f"Total figures: {total_figures}")
with col_info3:
    render_time = time.time() - st.session_state.last_time
    st.info(f"Render time: {render_time:.3f}s")
with col_info4:
    try:
        data_shape = str(data[st.session_state.current_index].shape)
        st.info(f"Current data shape: {data_shape}")
    except Exception:
        st.info("Current data shape: N/A")
# Navigation controls
...
with col4:
    # Additional feature: Jump to random figure
...
st.caption(f"Figure {st.session_state.current_index + 1} of {total_figures} " +
          f"(Index: {st.session_state.current_index})")
