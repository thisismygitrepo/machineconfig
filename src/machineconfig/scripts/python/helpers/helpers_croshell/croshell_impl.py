...
def _build_fire_line(
    ...
    if visidata:
        ...
        if Path.home().joinpath("code/machineconfig").exists():
            ...
            requirements = f"""{uv_python_line} {user_uv_with_line} {uv_project_line} --with "marimo,sqlglot,cowsay,machineconfig[plot]>=8.53" """
        fire_line = f"""
...
"""
        ...
        if Path.home().joinpath("code/machineconfig").exists():
            ...
            requirements = f"""{user_uv_with_line} {uv_project_line} --with "cowsay,machineconfig[plot]>=8.53" """
...
uv venv
uv add "cowsay,machineconfig[plot]>=8.53"
uv add {user_uv_add}
# code serve-web
...
"""
        ...
        if Path.home().joinpath("code/machineconfig").exists():
            ...
            ve_line = f"""{uv_python_line} {user_uv_with_line} {uv_project_line} --with "cowsay,machineconfig[plot]>=8.53" """
