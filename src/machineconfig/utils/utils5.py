...
class Cache(Generic[T]):
    def __init__(self, source_func: Callable[[], T],
                 expire_ms: int, logger: Optional[PrintFunc] = None, path: Optional[Path] = None,
    ...
    def __setstate__(self, state: dict[str, Any]) -> None:
        ...
        self.path = Path.home() / self.path if self.path is not None else self.path
    def __getstate__(self) -> dict[str, Any]:
        state = self.__dict__.copy()
        state["path"] = self.path.relative_to(Path.home()) if self.path is not None else state["path"]
        return state
...
════════════════════════════════════════════════════════════""")
        return self.cache
@staticmethod
def as_decorator(expire: int = 60000, logger: Optional[PrintFunc] = None, path: Optional[Path] = None,
                 name: Optional[str] = None):
    def decorator(source_func: Callable[[], T2]) -> Cache['T2']:
        res = Cache(source_func=source_func, expire_ms=expire, logger=logger, path=path, name=name)
        return res
    return decorator
