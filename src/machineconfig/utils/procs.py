    ...
    def kill(self, names: list or None = None, pids: list or None = None):
        ...
        for name in names:
            ...
            if len(rows) > 0:
                for pid in rows['pid'].values:
                    ...
                    print(f'Killed process {name} with pid {pid}. It lived {pd.Timestamp.now() - pd.to_datetime(rows["create_time"].values[0], unit="s")}.')
            else:
        ...
        for pid in pids:
            try:
                ...
                proc.kill()
                print(f'Killed process with pid {pid} and name {proc.name()}.  It lived {pd.Timestamp.now() - pd.to_datetime(proc.create_time(), unit="s")}.')
